...
exports.signup = function (req, res) {
  ...
  User.findOne({email: req.body.email}, function(err, userPresent) {
    if (err) {
      ...
      if(userPresent && userPresent._id) {
        if(req.body.isOtherLogin === true || req.body.isOtherLogin  == 'true') {
          return res.send(userPresent);
        } else {
          return res.status(402).send({message: 'This email already exists'});          
        }
      } else {
        //Remove the securty purpose data
